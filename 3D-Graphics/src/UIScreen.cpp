	...
	,mState(EActive)
{
	// Add to UI Stack
	mGame->PushUI(this);
	mFont = mGame->GetFont("Assets/Carlito-Regular.ttf"); // Carlito-Regular.ttf , Arial.ttf
...
{
	// Scale the quad by the width/height of texture and flip y if need to
	float yScale = static_cast<float>(texture->GetHeight()) * scale;
	if (flipY) { yScale *= -1.0f; }

	Matrix4 scaleMat = Matrix4::CreateScale(
		static_cast<float>(texture->GetWidth()) * scale,
		yScale, //static_cast<float>(texture->GetHeight()) * scale,
		1.0f);
