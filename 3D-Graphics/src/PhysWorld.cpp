...
void PhysWorld::TestPairwise(std::function<void(Actor*, Actor*)> f)
{
	...
	for (size_t i = 0; i < mBoxes.size(); i++)
	{
		...
		{
			BoxCollisionComponent* a = mBoxes[i];
			BoxCollisionComponent* b = mBoxes[j];
...
void PhysWorld::AddBox(BoxCollisionComponent* box)
{
	mBoxes.emplace_back(box);
}

void PhysWorld::RemoveBox(BoxCollisionComponent* box)
{
