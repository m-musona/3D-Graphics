	...
	:Actor(game)
{
	...
	mBoxCollComp = new BoxCollisionComponent(this);
	AxisAlignedBoundingBox myBox(Vector3(-25.0f, -25.0f, -87.5f), Vector3(25.0f, 25.0f, 87.5f));
	mBoxCollComp->SetObjectBox(myBox);
	mBoxCollComp->SetShouldRotate(false);
}
...
void FPSActor::FixCollisions()
{
	...
	const AxisAlignedBoundingBox& playerBox = mBoxCollComp->GetWorldBox();
	...
	for (auto pa : planes)
	{
		...
		const AxisAlignedBoundingBox& planeBox = pa->GetBox()->GetWorldBox();
		if (Intersect(playerBox, planeBox))
		{
			...
			SetPosition(pos);
			mBoxCollComp->OnUpdateWorldTransform();
		}
	}
}
